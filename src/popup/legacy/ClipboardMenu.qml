...
PlasmaComponents3.ScrollView {
    ...
    signal pasteSelection(var uuids)
    ...
    required property Item dialogItem
    ...
    function isMultiSelected(uuid): bool {
        return selectedUuids.some(entry => entry.uuid === uuid);
    }

    function selectedUuidList(): var {
        return selectedUuids.map(entry => entry.uuid);
    }

    function toggleMultiSelection(uuid, row): void {
        const next = selectedUuids.slice();
        const existing = next.findIndex(entry => entry.uuid === uuid);
        if (existing >= 0) {
            next.splice(existing, 1);
        } else {
            next.push({uuid: uuid, row: row});
    ...
    property Item header: PlasmaComponents3.ToolBar {
        bottomPadding: tabBar.visible ? 0 : Kirigami.Units.smallSpacing
